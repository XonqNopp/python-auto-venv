...
def venvauto():
    ...
    """
    venv_autouse_instance = file.VenvAutouse()
...
def test_get_filename_from_caller() -> None:
    ...
    assert file.VenvAutouse.get_filename_from_caller(currentframe()) == __file__
...
def test_digest_file_nonexistent() -> None:
    ...
    assert file.VenvAutouse.digest_file(FOO_FILE) == ''


def test_digest_file_existing() -> None:
    """ digest_file with existing file"""
    assert file.VenvAutouse.digest_file(SAMPLE_REQ_FILE) == SAMPLE_REQ_FILE_HASH
